...
public class UploadUtil {
	...
	private static final String PREFIX = "--";   //字段的开始标识符 必须是--
...
//					dos.flush();
		...
		//	this.requestTime = (int) ((responseTime-requestTime)/1000);
	     Log.e(TAG, "服务器返回response code:" + res);
			if (res == 200) {
				Log.e(TAG, "提交成功");
				...
				result = sb1.toString();
				Log.e(TAG, "服务器返回信息 : " + result);
				onUploadProcessListener.onUploadDone(SUCCESS, result);
				return;
			} else {
				Log.e(TAG, "request error");
				onUploadProcessListener.onUploadDone(SERVERERROR,"上传失败：code=" + res);
				return;
			}
		} catch (MalformedURLException e) {
			onUploadProcessListener.onUploadDone(SERVERERROR,"上传失败：error=" + e.getMessage());
			e.printStackTrace();
			return;
		} catch (IOException e) {
			onUploadProcessListener.onUploadDone(SERVERERROR,"上传失败：error=" + e.getMessage());
...
/*	//发送上传结果
	...
	{
		Log.v("zms", "UploadUtil状态吗:"+responseCode+"信息:"+responseMessage);
